...
// nlopt, timer and log includes
#include "nlopt-2.4.2-dll64\nlopt.h"
#include "HighResTimer_class.hpp"
#include "logs.cpp"

///////////////////////////////////////
// 2. macros realated to Python link //
///////////////////////////////////////
...
double objfunc(unsigned n, const double *x, double *grad, void *solver_data_in)
{
    ...
    // b. set number of repetitions
    int rep = 5000000;

    // c. initialize solver struct and load data from solver input
    solver_struct *solver_data = (solver_struct *) solver_data_in;
    double alpha = solver_data->alpha;
    double beta  = solver_data->beta;

    // d. gradient of the objective function
    if (grad) {
    ...
    // f. update solver data
    solver_data->evals++;
    solver_data->time += timer.StopTimer();

    // g. return objective function. Negative since we are minimizing
    return -obj;
...
double ineq_constraint(unsigned n, const double *x, double *grad, void *solver_data_in)
{
    ...
    // b. initialize solver data
    solver_struct *solver_data = (solver_struct *) solver_data_in;

    // c. gradient of the constraint
    if (grad) {
        grad[0] = 1.0;
        grad[1] = 1.0;
    }
...
EXPORT void optimize() // the EXPORT macro is used to make the function callable from Python
{
